...
$$EVCA = \underbrace{\sum_{k=1}^K p(M_k) \cdot \max_d EU(d|M_k)}_{\text{With perfect information}} - \underbrace{\max_d \sum_{k=1}^K p(M_k) \cdot EU(d|M_k)}_{\text{BMA (under ambiguity)}}$$
...
The yellow border highlights the optimal decision under BMA. Notice how different decisions perform best under different models—this is why EVCA is positive.
...
5. **Make decision**:
   - If EVCA < research cost: Choose optimal decision under BMA
...
- Low EVCA → Optimal decision is robust; proceed with BMA
- EVCA integrates Bayesian model averaging with decision theory
